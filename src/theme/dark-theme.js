export default {
  '--bg-color': '#252525',
  '--bg-color-2': '#181818',
  '--divider-color': 'rgba(107,72,72,0.55)',  /* 分割线 */
  '--divider-text-color': 'rgba(163,222,220,0.55)',  /* 分割字体颜色 */

  '--text-color': 'rgb(239,233,233)',
  '--text-color-2': 'rgb(185,179,179)',
  '--text-active-color': 'rgb(144,238,159)',

  '--error-color': '#b57ce3' /* 错误消息颜色 */,
  '--success-color': '#10d02e' /* 成功消息颜色 */,

  '--a-color': 'rgb(99, 253, 164)',
  '--a-color-2': 'rgb(152, 255, 190)',

  '--popper-bg-color': '#424242',  /* 弹出框背景颜色 */
  '--popper-hover-bg-color': '#595959',  /* 弹出框悬浮背景色 */
  '--popper-border-color': '#2f2f2f',  /* 弹出框边框颜色 */

  '--switch-bg-color': '#424242',  /* 开关背景颜色 */
  '--switch-active-bg-color': '#424242',  /* 开关激活颜色 */
  '--switch-disable-bg-color': '#ffffff',  /* 开关禁用颜色 */
  '--switch-border-color': '#2f2f2f',  /* 开关边框颜色 */
  '--switch-btn-bg-color': 'rgb(239,233,233)',  /* 开关按钮颜色 */

  '--navbar-color': 'rgba(235, 235, 235)',
  '--navbar-bg-color': '#181818',
  '--redirect-card-bg-color': 'rgba(24, 24, 24, 1.0)',

  '--button-bg-color': '#424242',  /* 按钮背景色 */
  '--button-border-color': '#2f2f2f',  /* 按钮边框颜色 */
  '--button-text-color': 'rgb(239,233,233)',  /* 按钮字体颜色 */

  '--button-danger-bg-color': '#e10c54',  /* 删除按钮背景色 */
  '--button-danger-border-color': '#e10c54',  /* 删除按钮边框颜色 */
  '--button-danger-text-color': 'rgb(239,233,233)',  /* 删除按钮字体颜色 */
}

