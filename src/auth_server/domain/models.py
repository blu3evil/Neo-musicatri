...
class Role(db.Model):
    ...
    disabled = db.Column(db.Boolean, nullable=False, default=False)  # 此权限等级是否禁止修改
...
def init_roles():
    def ensure_role(name, desc='', disabled=False):
        role = Role.query.filter_by(name=name).first()
        if not role:
            role = Role(name=name, description=desc, disabled=disabled)
            db.session.add(role)

    ensure_role('admin', 'Administrator role')
    ensure_role('user', 'Normal user role', True)
    ensure_role('anonymous', 'Anonymous user role', True)
    db.session.commit()
...
def model_to_dict(instance,
                  ...
                  exclude: set[str] = None) -> dict:
    """ 将sqlalchemy模型实例转换为字典，此方法支持协同外键一并转换，通过递归调用实现 """
    if not isinstance(instance, db.Model):
        return {}  # 非实体类对象直接返回空字典

    result = {}
    exclude = exclude or set()  # 排除项

    for column in instance.__table__.columns:  # 处理普通字段
        if column.name not in exclude:
            result[column.name] = getattr(instance, column.name)

    if include_relationships:  # 处理关联字段
        for rel in instance.__mapper__.relationships:
            ...
            if value is None:
                result[rel.key] = None
            elif rel.uselist:  # 多对多、一对多时字段为列表
                result[rel.key] = [model_to_dict(item, include_relationships=False) for item in value]
            else:  # 一对一、多对一时字段为对象
                result[rel.key] = model_to_dict(value, include_relationships=False)
