...
from auth_server.services import auth_service
...
def init(socketio: SocketIO):
    ...
    def user_connect():
        ...
        if result.code != 200:
            ...
            logger.info(f'user socket connected, user sid: {session.get(session_sid_name)}')
            emit(SocketioEvent.CONNECT_ACCEPT, result.as_dict())

    @socketio.on('disconnect', namespace='/socket/user')
    def user_disconnect():
        """ 客户端中断websocket连接"""
        logger.info(f'user socket disconnected, user sid: {session.get(session_sid_name)}')
