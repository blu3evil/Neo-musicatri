...
def info():
    ...
    return jsonify({
        'data': {
            'name': config.get(AuthServerConfigKey.APP_INFO_NAME),
            'version': config.get(AuthServerConfigKey.APP_INFO_VERSION),
            'description': config.get(AuthServerConfigKey.APP_INFO_DESCRIPTION),
        }
    })  # 成功获取服务器数据