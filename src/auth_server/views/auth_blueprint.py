...
from auth_server.auth_server_context import context, AuthServerConfigKey
from auth_server.services.auth_service import auth_service
from auth_server.services.session_service import session_service
...
redirect_uri = config.get(AuthServerConfigKey.DISCORD_OAUTH_REDIRECT_URI)
...
def login():
    ...
    if result.code != 200: return result.as_response()

    # 更新当前用户的userid 到session key的映射
    result = session_service.upsert_user_session(user_id=result.data['user_id'])
...
def verify_role(role):
    ...
    result = auth_service.verify_role(result.data['user_id'], role)  # 校验用户权限
    return result.as_response()
...
def logout():
    ...
    result = auth_service.user_logout(result.data['user_id'])
