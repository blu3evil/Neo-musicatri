...
export default {
  ...
  setup() {
    ...
    const handleRowClick = async (row, column, event) => {
      ...
      let userId = row.id
      let result = await adminUserService.getUserDetails(userId)
      if (!result.isSuccess()) {
        // 请求用户详情失败
        return
      }
      ...
      // 加载可用权限级别
      result = await adminUserService.getAllRoles()
      if (!result.isSuccess()) {
        // 权限级别请求失败
        return
      }
    ...
    const handleUserDelete = () => {
      ...
      PopupMessage.warning(t('view.workspace.UserManagement.delete_user_or_not', {
        ...
        // 确认删除用户
        const result = await adminUserService.deleteUser(targetUserForm.id)
        if (result.isSuccess()) {
          // 删除成功
      ...
      if (!compareTargetUserForm(targetUserFormCache, targetUserForm)) {
        ...
        PopupMessage.warning(t('view.workspace.UserManagement.submit_change_or_not'))
          .then(async () => {
            ...
            // 提交patch请求修改用户数据
            const result = await adminUserService.patchUser(targetUserForm.id, {
    ...
    async function refreshUserTable() {
      ...
      const result = await adminUserService.getUsersPreview(formInline) // 加载用户数据概览
      if (result.isSuccess()) {
        // 成功获取用户概览数据
...
<!--    </template>-->
  ...
  <CommonPanel style="height: 55px">
    <el-form :inline="true" :model="formInline" class="user-table-search">
      <el-form-item :label="globalNameLabel">
        <el-input
          v-model="formInline.global_name"
