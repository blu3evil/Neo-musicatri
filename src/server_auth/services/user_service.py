...
from server_auth.domain.models import DiscordUser, Role, UserRole, to_dict, db
from server_auth.services.cache_service import cache_service
from server_auth.services.session_service import session_service
...
class UserService:
    ...
    @staticmethod
    def get_users_preview(condition: dict):
        """ 获取全部用户简要信息，用于前端表格展示 """
    ...
    def update_user(user_id: int, data: dict):
        ...
        cache_service.clear_user(user_id)
    ...
    def delete_user(user_id: int):
        ...
        result = session_service.unregister_user_session(user_id=user_id)  # 注销用户会话
        if result.code != 200: return result

        result = cache_service.clear_user(user_id=user_id)  # 清理用户数据缓存
        if result.code != 200: return result
